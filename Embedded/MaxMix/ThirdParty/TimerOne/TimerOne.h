...
public:
	void initialize(unsigned long microseconds=1000000) __attribute__((always_inline)) {
		esp_timer_init();
		auto c_args = esp_timer_create_args_t();
		c_args.name = "TimerOne";
		esp_timer_create(&c_args, &tHandle);
		setPeriod(microseconds);
	}
	void setPeriod(unsigned long microseconds) __attribute__((always_inline)) {
		// Limitation of system that periods be slower than 50us
		period = max(50UL, microseconds);
	}
    ...
    void stop() __attribute__((always_inline)) {
		esp_timer_stop(tHandle);
    }
    void restart() __attribute__((always_inline)) {
	start();
    }
    void resume() __attribute__((always_inline)) {
		esp_timer_start_periodic(tHandle, period);
    }
    ...
    void attachInterrupt(void (*f)()) __attribute__((always_inline)) {
		auto c_args = esp_timer_create_args_t();
		c_args.name = "TimerOne";
		c_args.callback = reinterpret_cast<esp_timer_cb_t>(f);
		isrCallback = f;
		esp_timer_create(&c_args, &tHandle);
		resume();
    }
